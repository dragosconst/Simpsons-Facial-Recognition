...
"""
def evaluate_detections_facial(detections, scores, gt, valid):
...
def evaluate_detections_classes(detections, scores, gt, valid):
    print("other stuff")
    true_detections = np.zeros(len(detections))
    false_detections = np.zeros(len(detections))
    gt_exists_detection = np.zeros(len(gt))
    ...
    for d_index, detection in enumerate(detections):
        *box, det_class, im_index = detection
        max_overlap = -1
        max_index = None
        for gt_index, tuples in enumerate(gt):
            g_index, *g_box, im_class = tuples
            ...
            # here we have one of the true detections, let's check it against the current detection
            overlap = intersection_over_union(box, g_box)
            if overlap > max_overlap and im_class == det_class:
                # print(g_index, g_box, im_class)
                print(overlap)
                xd1, yd1, xd2, yd2 = box
                x1, y1, x2, y2 = g_box
    ...
    plt.ylabel('Precision')
    plt.title('Average precision %.3f' % average_precision)
    plt.savefig(os.path.join("data/", 'precizie_medie_detectie_fete.png'))
    plt.show()